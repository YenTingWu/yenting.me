...
During my self-taught period and my last job, I had learned and experienced UI techniques and built a fundamental mindset of how to build a comprehensive client-side applications. However, when talking about the server side, I had a slightly understanding. I had been curious about the detail of how both sides communicate; been frustrated when cors problems showed up somehow, which took me tons of time to find out it just needed to add a fucking one-line code into the backend codebase and the error solved; felt alienated when other people were talking about api gateway, docker and other backend stuff. These experiences cultivated my curiosity and pushed me to learn more things.

After leaving my last job, I took a course - [Stanford - CS 253 Web Security](https://web.stanford.edu/class/cs253) instructed by Feross Aboukhadijeh, who was such a great instructor and a tech enthusiast. I can still feel his passion and how humble he was even though he had been in this industry for decades. For me, He was **Jack Peralta** in tech. Just give [the annoying website](https://theannoyingsite.com/) a try **_(You might like to use another browser instead of the current one you are using)_** and you will know why.

In this course, he took us through the comprehensive overview of web security. He demoed how people attack a site and demoed again how we could prevent our sites from these malicious attacks. Also, he went into detail about basic concepts of web, including **dns, https, tls and the different version of auth**, which are terms I had heard before but never fully understood. After his explaination, I would not say I'm an expert of these concepts, the water is too deep, but at least I can tell which role they are playing in the whole network. This actually helps a lot because you start to realize what you're working on belongs to which component in a huge picture.

While taking this course, I started browsing around new things in the web community just for satisfying my curiosity. I worked so hard last year in order to get a job in the shortest time so that I had no time to try anything that was not a job requirement. Besides, I had no ability to understand those discussions, and had no experience to tell which solution was more appropiate to a specific situation or which issue the library solved. It was so much pleasure that you can understand what people were discussing even though it was hard to grasp all the points at the first place in some cases. At this point, I finally realized what **"trade off"** is. It was a term that Jimmy, my ex-manager, had reminded me several times but I couldn't really get it. **_Not knowing "trade off" had screwed me up beyond number not only in programming but in my life and my work as well._**

Before this studying time, it had never been so happy for me. I tried bunch of libraries, learned more about computers and web, and joined an open-source project even though all I can help was to translate english to traditional mandarine, but I got a chance to see how people worked together in a remote workspace, which is also my dream pattern for work. However, I wasn't sure enough what I had learned so far if I only input my knowledge without having an output.So I decided to make a small project to play with what I had learned and try some libraries I'd like to try. In this project, my goal was:
...
- It provides apis to improve the optimization, including [Image tag](https://nextjs.org/docs/api-reference/next/image) and [AMP support](https://nextjs.org/docs/advanced-features/amp-support/introduction) and some for [flexible data fetching](<(https://nextjs.org/docs/basic-features/data-fetching)>) in different situations.
...
I have not much experience with ORM and SQL. It was really confusing for me that people used different apis to do literally the same thing. I didn't know what I should follow, so whatever was workable was fine for me and both patterns are in my codebase now. Apprentlly, I didn't read the doc clearly. While writing this article, I found out there are two different patterns, [Active Record and Data Mapper](https://typeorm.io/#/active-record-data-mapper/which-one-should-i-choose).

This is my first time of using ORM. I've only used Knex as database before and I think:
...
But I actually don't like TypeORM because the lakc of docs. [Prisma](https://www.prisma.io/) might be the next ORM I would like to play around with.
...
It was a fun experience to build an entire app from scratch, from select tech stack and design the layout and UI, to actually build out the app. Although it really took me a lot of time being confused, I've not only reached the goal I set before starting:

- Build an app with server and client sides
- Build authentication with JWT and Google API

but I've also gone through the whole process in a web developement on my own after leaving the first job. It really helps me rethink about how important every role playing in a project is.

Apart from things mentioned above, I'd like to highlight out few points I must do better next time:
...
- **Goal Setting**
  ...
  I have to remind future me of making goal **more simple** and **more specific**.
